...
class TestTableMetadata(unittest.TestCase):
    ...
    def test_table_description_metadata_for_legacy_manifest_is_valid(self):
        ...
        table_metadata = [
            {"key": "KBC.description", "value": "Description of table"},
    ...
    def test_table_description_metadata_for_manifest_is_valid(self):
        ...
        table_metadata = {"KBC.description": "Description of table", "custom_key": "custom_value"}
...
class TestTableDefinition(unittest.TestCase):
    ...
    def test_out_old_to_new_has_headers_sliced(self):
        sample_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data_examples", "data1", "in", "tables"
        )
    ...
    def test_table_manifest_minimal(self):
        table_def = TableDefinition(
            "testDef", "somepath", is_sliced=False, columns=["foo", "bar"], primary_key=["foo", "bar"]
        )
        self.assertEqual(
            {
                "columns": ["foo", "bar"],
    ...
    def test_table_manifest_missing_key(self):
        with self.assertRaises(UserException) as e:
            TableDefinition("testDef", "somepath", is_sliced=False, primary_key=["foo", "bar"])

        self.assertEqual(
            str(e.exception), "Primary key column foo not found in schema. Please specify all columns / schema"
        )
    ...
    def test_build_from_manifest_matching_table_valid_attributes(self):
        ...
        expected_table_def = TableDefinition(
            name="sample.csv", full_path=os.path.join(sample_path, "sample.csv"), is_sliced=False
        )
    ...
    def test_build_from_manifest_orphaned_table_valid_attributes(self):
        ...
        expected_table_def = TableDefinition(
            name="orphaned.csv", full_path=os.path.join(sample_path, "orphaned.csv"), is_sliced=False, incremental=False
        )
    ...
    def test_build_from_manifest_sliced_table_valid_attributes(self):
        ...
        expected_table_def = TableDefinition(
            name="sliced.csv", full_path=os.path.join(sample_path, "sliced.csv"), is_sliced=True
        )
    ...
    def test_build_from_manifest_orphaned_manifest_valid_attributes(self):
        ...
        expected_table_def = TableDefinition(name="orphaned_manifest.csv", full_path=None, is_sliced=False)
    ...
    def test_build_manifest_new_to_legacy_none_metadata_skipped(self):
        table_def = TableDefinition(
            ...
            destination="some-destination",
            incremental=True,
            schema={"foo": ColumnDefinition(metadata={"KBC.description": None, "some": "value"})},
        )
    ...
    def test_table_manifest_error_column_delete_2(self):
        with self.assertRaises(TypeError):
            TableDefinition("testDef", "somepath", is_sliced=False, delete_where={"column": "a", "values": "b"})

    def test_table_manifest_error_column_delete_3(self):
        with self.assertRaises(TypeError):
            TableDefinition(
                "testDef", "somepath", is_sliced=False, delete_where={"column": "a", "values": "b", "operator": "c"}
            )
    ...
    def test_unsupported_legacy_queue_properties_log(self):
        with self.assertLogs(level="WARNING") as log:
            ...
            self.assertIn(
                "WARNING:root:Running on legacy queue some manifest properties will be ignored: ['write_always']",
                log.output[0],
            )
    ...
    def test_new_manifest_native_types(self):
        ...
        # update column
        table_def.update_column("foo", ColumnDefinition(data_types=BaseType.integer(length="20")))
        ...
        table_def.add_column("id", ColumnDefinition(primary_key=True, data_types=BaseType.numeric(length="200")))
        ...
        self.assertDictEqual(
            {
                ...
                "delete_where_operator": "eq",
                "schema": [
                    {"name": "foo", "data_type": {"base": {"type": "INTEGER", "length": "20"}}, "nullable": True},
                    ...
                    {
                        "name": "id",
                        "data_type": {"base": {"type": "NUMERIC", "length": "200"}},
                        "nullable": True,
                        "primary_key": True,
                    },
        ...
        self.assertDictEqual(
            {
                ...
                "schema": [
                    ...
                    {
                        "name": "id",
                        "data_type": {"base": {"type": "INTEGER", "length": "200"}},
                        "nullable": True,
                        "primary_key": True,
                    },
                ],
        ...
        self.assertDictEqual(
            {
                ...
                "delete_where_operator": "eq",
                "schema": [
                    {"name": "foo", "data_type": {"base": {"type": "INTEGER"}}, "nullable": True},
                    {
                        "name": "bar",
                        "data_type": {"base": {"type": "STRING"}, "redshift": {"type": "STRING", "length": "255"}},
                        "nullable": True,
    ...
    def test_build_from_manifest_full_input(self):
        sample_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data_examples", "data_full_input_manifest", "in", "tables"
        )
    ...
    def test_build_from_manifest_full_output(self):
        sample_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data_examples", "data_full_output_manifest", "in", "tables"
        )
...
class TestFileDefinition(unittest.TestCase):
    ...
    def test_file_manifest_minimal(self):
        ...
        self.assertDictEqual(
            {"is_public": False, "is_permanent": False, "is_encrypted": False, "notify": False},
    ...
    def test_file_output_manifest_ignores_unrecognized(self):
        ...
        self.assertDictEqual(
            {"is_encrypted": True, "is_permanent": False, "is_public": False, "notify": False, "tags": ["dilbert"]},
            file_def.get_manifest_dictionary(),
        )
    ...
    def test_build_from_manifest_matching_file_valid_attributes(self):
        ...
        file_def = FileDefinition.build_from_manifest(manifest_path)
    ...
    def test_build_from_manifest_nonexistentfile_fails(self):
        sample_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data_examples", "data1", "in", "files")
    ...
    def test_user_tags(self):
        ...
        file_def = FileDefinition(
            "123_test_Def", is_permanent=True, is_encrypted=True, is_public=True, tags=all_tags, notify=True
        )

        self.assertDictEqual(
            {"tags": all_tags, "is_public": True, "is_permanent": True, "is_encrypted": True, "notify": True},
            file_def.get_manifest_dictionary(),
        )
    ...
    def test_all_tags(self):
        ...
        file_def = FileDefinition("123_test_Def", tags=all_tags)

        self.assertEqual(all_tags, file_def.tags)

    def test_build_from_manifest_s3_staging(self):
        sample_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data_examples", "data_storage_staging_s3", "in", "tables"
        )
    ...
    def test_build_from_manifest_abs_staging(self):
        sample_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data_examples", "data_storage_staging_abs", "in", "tables"
        )
