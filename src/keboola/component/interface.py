...
def init_environment_variables() -> dao.EnvironmentVariables:
    ...
    return dao.EnvironmentVariables(
        data_dir=os.environ.get("KBC_DATADIR", None),
...
class CommonInterface:
    ...
    def __init__(self, data_folder_path: str = None, log_level=logging.INFO, logging_type=None):
        ...
        # init logging
        logging_type_inf = (
            CommonInterface.LOGGING_TYPE_GELF
            if os.getenv("KBC_LOGGER_ADDR", None)
            else CommonInterface.LOGGING_TYPE_STD
        )
        if not logging_type:
        ...
        if not os.path.exists(data_folder_path) and not os.path.isdir(data_folder_path):
            raise ValueError(
                f"The data directory does not exist, verify that the data directory is correct. Dir: {data_folder_path}"
            )
    ...
    def _get_data_folder_from_context(self):
        ...
        argparser.add_argument("-d", "--data", dest="data_dir", default="", help="Data directory")
    ...
    @staticmethod
    def set_gelf_logger(
        log_level: int = logging.INFO, transport_layer="TCP", stdout=False, include_extra_fields=True, **gelf_kwargs
    ):  # noqa: E301
        """
    ...
    def get_input_table_definition_by_name(self, table_name: str) -> dao.TableDefinition:
        ...
        Returns:
            dao.TableDefinition
        """
        manifest_path = os.path.join(self.tables_in_path, table_name + ".manifest")
    ...
    def get_input_tables_definitions(self, orphaned_manifests=False) -> list[dao.TableDefinition]:
        ...
        table_files = [
            f for f in glob.glob(self.tables_in_path + "/**", recursive=False) if not f.endswith(".manifest")
        ]
        table_defs = list()
        ...
        if orphaned_manifests:
            ...
            manifest_files = [
                f
                for f in glob.glob(self.tables_in_path + "/**.manifest", recursive=False)
    ...
    def create_in_table_definition(
        ...
        return self._create_table_definition(
            name=name,
            storage_stage="in",
            is_sliced=is_sliced,
            destination=destination,
    ...
    def get_input_files_definitions(
        ...
        in_files = [f for f in glob.glob(self.files_in_path + "/**", recursive=False) if not f.endswith(".manifest")]
        file_defs = list()
        for t in in_files:
            manifest_path = t + ".manifest"
    ...
    def create_out_file_definition(
        ...
        return self._create_file_definition(
            name=name,
            storage_stage="out",
            tags=tags,
            is_public=is_public,
    ...
    def validate_image_parameters(self, mandatory_params):
        ...
        return self._validate_parameters(
            self.configuration.image_parameters, mandatory_params, "image/stack parameters"
        )
    ...
    def _validate_parameters(self, parameters, mandatory_params, _type):
        ...
        if missing_fields:
            raise UserException("Missing mandatory {} fields: [{}] ".format(_type, ", ".join(missing_fields)))
    ...
    def write_manifest(
        ...
        ** Usage:**
        ...
        manifest = io_definition.get_manifest_dictionary(
            legacy_queue=self.is_legacy_queue, legacy_manifest=legacy_manifest
        )
    ...
    def _expects_legacy_manifest(self) -> bool:
        legacy_manifest = self._running_in_kbc and self.environment_variables.data_type_support not in (
            "authoritative",
            "hints",
        )
...
class Configuration:
    ...
    def tables_input_mapping(self) -> list[dao.TableInputMapping]:
        ...
        for table in tables_defs:
            # nested dataclass
            table["column_types"] = [
                dao.build_dataclass_from_dict(dao.TableColumnTypes, coltype)
                for coltype in table.get("column_types", [])
            ]

            im = dao.build_dataclass_from_dict(dao.TableInputMapping, table)
            im.full_path = os.path.normpath(os.path.join(self.data_dir, "in", "tables", table["destination"]))
            tables.append(im)
        return tables
